...
def setup_level(space, player_body):
    ...
    spawn_ball(
        space, player_body.position +
        (0, 40), random.choice([(1, 10), (-1, 10)])
    )
    ...
    h = space.add_collision_handler(
        collision_types["brick"], collision_types["ball"])
...
def main():
    ...
    h = space.add_collision_handler(
        collision_types["ball"], collision_types["bottom"])
    ...
    h = space.add_collision_handler(
        collision_types["player"], collision_types["ball"])
    ...
    while running:
        ...
        screen.blit(
            font.render("fps: " + str(clock.get_fps()),
        ...
        screen.blit(
            font.render(
                "Press R to reset, ESC or Q to quit", 1, pygame.Color(
                    "darkgrey")
            ),
            (5, height - 20),
        )
